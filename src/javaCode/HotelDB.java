 ...
 * method executes everything and the SQL database. The program exists as a way
 * for employees to check rooms specifications, create and delete reservations,
 * check in and out guests, print bills, and print two different reports.
 * 
	...
	public static void main(String[] args) throws IOException, SQLException, ClassNotFoundException {
		...
		try (Connection conn = SimpleDataSource.getConnection(); Statement stat = conn.createStatement()) {
			try {
				...
				// get exception if the tables don't exist yet
			}
			...
			// Inserts all data from hotel.txt into the Hotel table
			...
			// Inserts all data from calendar.txt into the Calendar table
			...
			// Employee interface; allows several different choices and repeats
			...
			while (continueProgram) {
				...
				// ROOMS
				// prints all of the room specifications from the Hotel table
				...
				// BILLS
				// prints the bill based on room number entered by the user.
				// Gets price information from the Hotel table
				...
				// CALENDAR
				// add, delete, check in/check out, print reservations from the
				// Calendar table and automatically updates the Calendar.txt
				// file
					...
					} // print Calendar table in program
					...
					// Write updates to the Calendar table to text file
				...
				// reports
				// prints two reports; occupancy rate and housekeeping report
				// write both reports to a separate text file once the function
				// is run
					...
					if (command.equals("A")) {
						...
						String output = "Total rooms needing housekeeping: " + housekeepingResult
								+ System.lineSeparator() + "Room Numbers:" + System.lineSeparator();
						while (printTable.next()) {
							int roomNum = printTable.getInt("roomNum");
